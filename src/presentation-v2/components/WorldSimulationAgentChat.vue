<template>
  <AcuPanel title="世界推演 Agent 会话" description="输入补充后由当前 T1–T9 runtime 取证、派工并提交；所有结果仍受锚点、revision 与 active swipe 校验。">
    <AcuMessage v-if="error" kind="error">{{ error }} <AcuButton size="sm" @click="emit('refresh')">重新读取</AcuButton></AcuMessage>
...
import type { WorldSimulationStagePlan_ACU, WorldSimulationTaskStatus_ACU } from '../../service/simulation/model';
...
const emit = defineEmits<{
  ...
  (event: 'cancel' | 'confirm-plan' | 'resume' | 'refresh'): void;
