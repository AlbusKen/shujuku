...
function assertIdentity_ACU(envelope: WorldSimulationEnvelope_ACU | null, chatIdentity: string, identity: WorldSimulationRunIdentity_ACU): void {
...
export class WorldSimulationStageExecutionEngine_ACU<TInput extends { identity: WorldSimulationRunIdentity_ACU }> {
  ...
  async run(input: TInput): Promise<WorldSimulationMainLoopResult_ACU> {
    assertIdentity_ACU(this.dependencies.readEnvelope(), this.dependencies.getChatIdentity(), input.identity);
