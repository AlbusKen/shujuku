...
export class WorldSimulationRuntime_ACU {
   ...
   */
  async repairPendingMaterials(
    modules: readonly import('./model').WorldSimulationLedgerModule_ACU[],
  ): Promise<WorldSimulationOrchestratorResult_ACU | null> {
    const allowed = new Set<import('./model').WorldSimulationLedgerModule_ACU>([
    ...
    if (!targets.length || targets.length !== new Set(modules).size) {
      throw new WorldSimulationValidationError_ACU(createWorldSimulationError_ACU(
        'WORLD_SIMULATION_SNAPSHOT_INVALID', 'agent_loop', '请选择至少一个合法的资料模块进行补足', false,
      ));
    }
    const chat = this.getChat();
    const chatIdentity = getActiveChatStorageIdentity_ACU(chat);
    if (!chatIdentity) {
      throw new WorldSimulationValidationError_ACU(createWorldSimulationError_ACU(
        'WORLD_SIMULATION_CHAT_UNAVAILABLE', 'agent_loop', '当前聊天不可用，无法补足世界资料', false,
      ));
    }
    if (this.orchestrator.isInFlight(chatIdentity)) {
      throw new WorldSimulationValidationError_ACU(createWorldSimulationError_ACU(
        'WORLD_SIMULATION_REVISION_CONFLICT', 'agent_loop', '世界推演正在运行，请等待本轮结束后再补足资料', true,
      ));
    }
    const envelope = this.readEnvelopeView_ACU();
    if (!envelope) {
      throw new WorldSimulationValidationError_ACU(createWorldSimulationError_ACU(
        'WORLD_SIMULATION_SNAPSHOT_INVALID', 'agent_loop', '世界账本尚未建立，不能执行资料补足', false,
      ));
    }
    ...
    if (ineligible.length) {
      throw new WorldSimulationValidationError_ACU(createWorldSimulationError_ACU(
        'WORLD_SIMULATION_REVISION_CONFLICT', 'agent_loop', `这些模块当前没有待补足缺口：${ineligible.join(', ')}`, false,
      ));
    }
    ...
    if (!anchor) {
      throw new WorldSimulationValidationError_ACU(createWorldSimulationError_ACU(
        'WORLD_SIMULATION_ANCHOR_STALE', 'anchor', '资料缺口对应的 assistant 锚点已不可恢复', false,
      ));
    }
    for (const module of targets) {
      ...
      if (frozen && (frozen.messageKey !== anchor.messageKey || frozen.swipeId !== anchor.swipeId || frozen.contentDigest !== anchor.contentDigest)) {
        throw new WorldSimulationValidationError_ACU(createWorldSimulationError_ACU(
          'WORLD_SIMULATION_ANCHOR_STALE', 'anchor', `模块 ${module} 的冻结锚点已变化，拒绝补足`, false,
        ));
      }
    }
    return pausedRun
